import os, sys, threading, cv2, rclpy
...
class YOLOCompressedSubscriber(Node):
    def __init__(self, model):
        super().__init__('yolo_compressed_subscriber')
        self.model = model
        self.bridge = CvBridge()
        self.queue = Queue(maxsize=1)
        self.should_shutdown = False
        self.class_names = model.names if hasattr(model, 'names') else ['Object']
    ...
    def run_detection(self):
        while not self.should_shutdown:
            ...
            cv2.imshow("YOLOv8 Compressed Detection", img)
...
def main():
    ...
    rclpy.init()
    node = YOLOCompressedSubscriber(model)
